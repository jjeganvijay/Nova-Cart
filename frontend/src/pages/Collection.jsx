    ...
    <div className='flex flex-col sm:flex-row gap-2 sm:gap-10 pt-10 border-t'>
      ...
      <div className='min-w-60'>
        <p onClick={()=>setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2'>
          FILTER OPTIONS
          <img className={`h-3 sm:hidden transition-transform duration-300 ${showFilter ? 'rotate-90' : ''}`} src={assets.dropdown_icon} alt="dropdown" />
        </p>

        {/* Category Filter */}
        <div className={`border border-gray-300 pl-5 py-3 mt-6 rounded-lg bg-gray-50 ${showFilter ? '' :'hidden'} sm:block`}>
        ...
        {/* SubCategory Filter */}
        <div className={`border border-gray-300 pl-5 py-3 my-5 rounded-lg bg-gray-50 ${showFilter ? '' :'hidden'} sm:block`}>
          <p className='mb-3 text-sm font-semibold'>PRODUCT TYPE</p>
          <div className='flex flex-col gap-2 text-sm font-normal text-gray-800'>
            <label className='flex gap-2 items-center'>
              <input className='w-3 h-3' type="checkbox" value={'Topwear'} onChange={toggleSubCategory}/> Topwear
            </label>
            <label className='flex gap-2 items-center'>
              <input className='w-3 h-3' type="checkbox" value={'Bottomwear'} onChange={toggleSubCategory}/> Bottomwear
            </label>
            <label className='flex gap-2 items-center'>
              <input className='w-3 h-3' type="checkbox" value={'Winterwear'} onChange={toggleSubCategory}/> Winterwear
            </label>
          </div>
        </div>
      </div>
